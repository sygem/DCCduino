...
void Sensor::check(){
  ...
  for(tt=firstSensor;tt!=NULL;tt=tt->nextSensor){
    if (tt->data.binary) {
      ...
      tt->active = true;
      int r = digitalRead(tt->data.pin);
      if (r > tt->signal+2 || r < tt->signal-1) {
        tt->signal = r;
        CommManager::printf("<J %d %d>",tt->data.snum, tt->signal);
      }
    }
...
Sensor *Sensor::create(int snum, int pin, int pullUp, int binary, int v){
  Sensor *tt;
  ...
  if(tt==NULL){       // problem allocating memory
    if(v==1)
      CommManager::printf("<X Sensor::create>");
    return(tt);
  }
...
void Sensor::remove(int n){
  ...
  if(tt==NULL){
    CommManager::printf("<X Sensor::remove>");
    return;
  }
...
void Sensor::show(){
  ...
  if(firstSensor==NULL){
    CommManager::printf("<X Sensor::show>");
    return;
  }
...
void Sensor::status(){
  ...
  if(firstSensor==NULL){
    CommManager::printf("<X Sensor::status>");
    return;
  }
...
void Sensor::parse(const char *c) {
  ...
  switch(sscanf(c,"%d %d %d %d",&n,&s,&m,&b)){

    case 4:                     // argument is string with id number of sensor followed by a pin number and pullUp indicator (0=LOW/1=HIGH)
      create(n,s,m,b,1);
    break;
    ...
    case 2:                     // invalid number of arguments
      CommManager::printf("<X Sensor::parse>");
      break;
  }
}
...
void Sensor::load(){
  ...
  for(int i=0;i<EEStore::eeStore->data.nSensors;i++){
    ...
    tt=create(data.snum,data.pin,data.pullUp,data.binary);
